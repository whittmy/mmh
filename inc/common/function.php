...
function getGameXmlText($f,$t,$v)
{
	$fromarr = explode("$$$",$v);
	$arr1 = $GLOBALS['MAC_CACHE']['game'.$t];
	$rc=false;
	$res="";
	for($i=0;$i<count($fromarr);$i++){
		if($rc){ $res.=","; }
...
function makeSelect($tabName,$colId,$colName,$colSort,$valUrl,$valspan,$valId)
{
	...
	switch($tabName)
	{
		...
		case "{pre}game_topic": $arr = $MAC_CACHE['gametopic']; break;
		default : $rc=false;
	}
	$res="";
	foreach($arr as $arr1){
...
function updateCacheFile()
{
	...
	*/
	...
	//游戏分类缓存
	try{
		$cachegametype= $db->queryarray('SELECT * FROM {pre}game_type','t_id');
		$i=0;
		foreach($cachegametype as $v){
			$strchild='';
			$rc=false;
			$rs= $db->query('SELECT t_id FROM {pre}game_type WHERE t_pid=' .$v['t_id']);
			while ($row = $db ->fetch_array($rs)){
				if($rc){ $strchild .=','; }
				$strchild .= $row['t_id'];
				$rc=true;
			...
			unset($rs);
			if (isN($strchild)){ $strchild = $v['t_id'];} else{ $strchild = $v['t_id'] . ',' . $strchild; }
			$cachegametype[$v['t_id']]['childids'] = $strchild;
				
			//$lnk = $tpl->getLink('game','type',$v,$row,false,1,1,false);
			//$cachegametype[$v['t_id']]['link'] = $lnk;
			//$plnk = '';
			//if($v['t_pid']>0){
			//	$plnk = $tpl->getLink('game','type',$cachegametype[$v['t_pid']],$row,false,1,1,false);
			//}
			//$cachegametype[$v['t_id']]['plink'] = $plnk;
			$i++;
		}
	}
	catch(Exception $e){
	...
	//游戏剧情分类缓存
	try{
		$cachegameclass=$db->queryarray('SELECT * FROM {pre}game_class','c_id');
	}
	catch(Exception $e){
		echo '更新游戏剧情分类缓存失败，请检查数据是否合法，是否包含引号、单引号、百分号、尖括号等特殊字符';
		exit;
	}
	$arr['gameclass'] = $cachegameclass;
	
	
	//游戏专题缓存
	try{
		$cachegametopic=$db->queryarray('SELECT * FROM {pre}game_topic','t_id');
	}
	catch(Exception $e){
		echo '更新游戏专题缓存失败，请检查数据是否合法，是否包含引号、单引号、百分号、尖括号等特殊字符';
		exit;
	}
	$arr['gametopic'] = $cachegametopic;	
...
function format_gamename($gamename){
	$gamename = str_replace(array('【','】','（','）','(',')','{','}'),array('[',']','[',']','[',']','[',']'),$gamename);
	$gamename = preg_replace('/\[([a-z][A-Z])\]|([a-z][A-Z])版/i','',$gamename);
	$gamename = preg_replace('/TS清晰版|枪版|抢先版|HD|BD|TV|DVD|VCD|TS|\/版|\[\]/i','',$gamename);
