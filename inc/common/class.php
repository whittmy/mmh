	...
	function Add($tabName,$arrFieldName ,$arrValue)
	{
		...
		if (chkArray($arrFieldName,$arrValue)){
			...
			foreach($arrValue as $b){
				if($rc){ $sqlval.=",";}
				$sqlval .= "'".  (MAC_MODULE=='home' ? mysql_real_escape_string($b) : $b) ."'";
				$rc=true;
			}
	...
	function Update($tabName,$arrFieldName , $arrValue ,$KeyStr,$f=0)
	{
		...
		if (chkArray($arrFieldName,$arrValue)){
			...
			for($i=0;$i<count($arrFieldName);$i++){
				if($rc){ $sqlval.=",";}
				if($f==0){
					$sqlval .= $arrFieldName[$i]."='". (MAC_MODULE=='home' ? mysql_real_escape_string($arrValue[$i]) : $arrValue[$i]) ."'";
				}
				else{
					$sqlval .= $arrFieldName[$i]."='". (MAC_MODULE=='home' ? mysql_real_escape_string($arrValue[$arrFieldName[$i]]) : $arrValue[$arrFieldName[$i]]) ."'";
				}
				$rc=true;
			}
