...
class AppTpl
{
    ...
    function AppTpl()
    {
		//类的成员变量不用声明，在函数中声明也是可以的,和你在类中直接用public $P 是没有区别的
		//动态产生的对象属性是全局性的，这与变量无需声明就可访问是一脉相承的
    	$this->P = array("vodtypeid"=>-1,
						 "vodtypepid"=>-1,
						 "vodtopicid"=>-1,
		    			"gametypeid"=>-1,
		    			"gametypepid"=>-1,
    ...
    function getDataCount($table,$flag)
	{
		switch($table)
		{
			...
			case 'game':
				$col = 'd_time';
				$sql = 'SELECT count(*) FROM {pre}game WHERE d_hide=0 AND d_type>0 ';
				break;				
			case 'art':
				$col = 'a_time';
    ...
    function mark_sql()
    {
		...
		switch($this->markname)
		{
			...
			case 'menu': 
				...
				else  if($lp['table']=='game'){
		        	$col .=',`t_tpl_game`, `t_tpl_game`, `t_tpl_down` ';
		        	if($GLOBALS['MAC']['user']['status']==1){ $where .= getTypeByPopedomFilter('menu'); }
				}
				else{
					$col .=',`t_tpl_art`';
			...
			case 'vod':
		        ...
		        if(!empty($lp['similar']) && is_array($this->P['similar'])){
		        	...
		        	if(count($tmp)>0){
			        	...
			        	if($rc){ $where .= " AND (" . $where2 . ") and d_id <> ". $this->D['d_id']." ";  }
		        	}
		        	unset($tmp);
		        }
				break;
			...
			case 'game':
				...
				switch($lp['by'])
		        {
		            ...
		            case "scorenum": $order .= " d_scorenum ";break;
		            case "rnd": 
		            	$datacount = $this->getDataCount('game','all');
            			$rand = mt_rand(0,$datacount-$lp['num']);
            			if($rand<0) { $rand=0; }
		            	$where .= ' AND d_id >=' .$rand ;
		            	$order .= " d_time ";
		        ...
		        if($auto==''){
		        	$this->P['pageflag'] = 'game';
		        	$limitstart = $lp['num'] * ($this->P['pg']-1);
		        	$limit = ' limit '.$limitstart.','. ($lp['num'] + $lp['start']);
		        	
		        	if ($this->P['gametypeid'] != -1){
		        		$typearr = $MAC_CACHE['gametype'][$this->P['gametypeid']];
		        		$expand='';
		            ...
		            elseif ($this->P['gametopicid'] != -1){
		                $where .= ' AND d_id IN(select r_b from {pre}game_relation where r_type=2 and r_a='.$this->P['gametopicid'].')';
		                $this->P['pagetype'] = 'topic';
		            }
		            elseif (!empty($this->P['ids'])){
		                $where .= ' AND d_id IN( ' . $this->P['ids']. ' )';
			        ...
			        if (!empty($lp['type'])){
				        if ($lp['type'] != 'all'){
				            if ($lp['type'] == 'current' && $this->P['gametypeid'] > -1){
				                $typearr = $MAC_CACHE['gametype'][$this->P['gametypeid']];
				                $where .= ' and d_type in (' . $typearr['childids'].')';
				            }
							else{
								if ( strpos ($lp['type'],',')>0){
									...
									$typearr = $MAC_CACHE['gametype'][$lp['type']];
									if(is_array($typearr)){
										$where .= ' and d_type in (' . $typearr['childids'] . ')';
									}
								}
			        ...
			        if (!empty($lp['topic'])){
				        if ($lp['topic'] != 'all'){
				            if ($lp['topic'] == 'current' && $this->P['gametopicid'] > -1){
				                $where .= ' AND d_id IN(select r_b from {pre}game_relation where r_type=2 and r_a='.$this->P['gametopicid'].')';
				                ' and d_topic in (' . $this->P['gametopicid'] . ')';
				            }
					        else{
				                $where .= ' AND d_id IN(select r_b from {pre}game_relation where r_type=2 and r_a in ('.$lp['topic'].'))';
				            }
				        }
			        }
		        }
				...
				if ($GLOBALS['MAC']['view']['gametype']!=2 && $GLOBALS['MAC']['user']['status']==1){
					$where .= getTypeByPopedomFilter('game');
				}
		        if (!empty($lp['starring'])){
		        	$where .= ' AND instr(d_starring,\''.$lp['starring'].'\')>0  ';
		        }
	...
	function base()
    {
    	...
    	array_push($colarr,'{maccms:link_map_game}');array_push($valarr,$this->getLink('game','map','',''));

    	array_push($colarr,'{maccms:link_topic_game}');array_push($valarr,$this->getLink('game','topicindex','',''));
    	...
    	array_push($colarr,'{maccms:link_search_game}');array_push($valarr,MAC_PATH."index.php?m=game-search");
    	...
    	if(strpos($this->H,"{maccms:count_game_all}")){ array_push($colarr,'{maccms:count_game_all}');array_push($valarr,$this->getDataCount('game',"all")); }
    	if(strpos($this->H,"{maccms:count_game_day}")){ array_push($colarr,'{maccms:count_game_day}');array_push($valarr,$this->getDataCount('game',"day")); }
    ...
    function parse($f, $mdes, $m1, $m2, $m3, $mrs, $mnum)
    {
        ...
        switch($f)
        {
            case "menu":
                switch($m2)
            	{
                    ...
                    case "count":
                        ...
                        else if($this->L['table'] == "game"){
                        	$typearr = $GLOBALS['MAC_CACHE']['gametype'][$mrs["t_id"]];
            ...
            case "vod":
            	...
            	case "game":
            		$typearr = $GLOBALS['MAC_CACHE']['gametype'][$mrs["d_type"]];
            		if (!is_array($typearr)){ return; }
            		$tp = $GLOBALS['MAC_CACHE']['gametype'][$typearr["t_pid"]];
            		...
            		switch($m2)
            		{
            			...
            			case "addtime": $val=getDatet($m3, $mrs["d_addtime"]);break;
            			case "time": $val=getDatet($m3, $mrs["d_time"]);break;
            			case "from": $val=getgameXmlText("gameplay","play",$mrs["d_playfrom"]);break;
            			case "fromdown": $val=getgameXmlText("gamedown","down",$mrs["d_downfrom"]);break;
            			case "link": $val=$this->getLink('game','detail',$typearr,$mrs);break;
            			case "playlink": $val=str_replace(array('{src}','{num}'),array('1','1'),$this->getLink('game','play',$typearr,$mrs));break;
            			case "playlinks":
            				$val='';
            				$arr = explode('$$$',$mrs['d_playfrom']);
            				$cc = count($arr);
            				if(empty($mrs['d_playfrom']) || $cc==0) { break; }
            				$xmlarr = $GLOBALS['MAC_CACHE']['gameplay'];
            				$url = $this->getLink('game','play',$typearr,$mrs);
            				for($i=0;$i<$cc;$i++){
            					$show = $xmlarr[$arr[$i]]['show'];
            					$lnk = str_replace(array('{src}','{num}'),array($i+1,'1'),$url);
            					$val.= '<a href="'.$lnk.'" target="_blank" class="playlink_'.$arr[$i].'">'.$show.'</a> ';
            				...
            				unset($arr,$xmlarr);
            				break;
            			case "downlink": $val=str_replace(array('{src}','{num}'),array('1','1'),$this->getLink('game','down',$typearr,$mrs));break;
            			case "downlinks":
            				$val='';
            				$arr = explode('$$$',$mrs['d_downfrom']);
            				$cc = count($arr);
            				if(empty($mrs['d_downfrom']) || $cc==0) { break; }
            				$xmlarr = $GLOBALS['MAC_CACHE']['gamedown'];
            				$url = $this->getLink('game','down',$typearr,$mrs);
            				for($i=0;$i<$cc;$i++){
            					$show = $xmlarr[$arr[$i]]['show'];
            					$lnk = str_replace(array('{src}','{num}'),array($i+1,'1'),$url);
            					$val.= '<a href="'.$lnk.'" target="_blank" class="downlink_'.$arr[$i].'">'.$show.'</a> ';
            			...
            			case "type": $val=$mrs["d_type"];break;
            			case "typepid": $val=$typearr["t_pid"];break;
            			case "typeplink": $val=$this->getLink('game','type',$tp,$mrs);break;
            			case "typepname": $val=$tp["t_name"];break;
            			case "typepenname": $val=$tp["t_enname"];break;
            			case "typepkey": $val=$tp["t_key"];break;
            			case "typepdes": $val=$tp["t_des"];break;
            			case "typelink": $val=$this->getLink('game','type',$typearr,$mrs);break;
            			case "typename": $val=$typearr["t_name"];break;
            			case "typeenname": $val=$typearr["t_enname"];break;
            			case "typekey": $val=$typearr["t_key"];break;
            			case "typedes": $val=$typearr["t_des"];break;
            			...
            			case "classlink" :
            				...
            				if(!empty($mrs['d_class'])){
            					...
            					foreach($ids as $a){
            						if(!empty($a)){
            							$arr = $GLOBALS['MAC_CACHE']['gameclass'][$a];
            							$mrs['id']=$typearr['t_id'];
            							$mrs['class']=$arr['c_id'];
            							$lnk = $this->getLink('game','list',$typearr,$mrs);
            							if($rc){ $val.='&nbsp;'; }
            							$val .="<a target='_blank' href='".$lnk."' />".$arr['c_name']."</a>";
            							$rc=true;
            						}
            			...
            			case "topiclink":
            				...
            				if(!empty($mrs['d_topic'])){
            					...
            					foreach($ids as $a){
            						if(!empty($a)){
            							$arr = $GLOBALS['MAC_CACHE']['gametopic'][$a];
            							$lnk = $this->getLink('game','topic',$arr,$mrs);
            							if($rc){ $val.='&nbsp;'; }
            							$val .="<a target='_blank' href='".$lnk."' />".$arr['t_name']."</a>";
            							$rc=true;
            						}
    ...
    function loadlist($flag)
    {
        ...
        if($flag == 'art'){
        	...
        	if($flag == 'game')
        		$classname =  $GLOBALS['MAC_CACHE']['gameclass'][$classid]['c_name'];
        	else
    ...
    function loadvod($flag)
    {
        if($this->P['make']!=true && chkCache($this->P['cp'],$this->P['cn'])){
            ...
            $this->P['similar'] = array('starring'=>$this->D['d_starring'],'tag'=>$this->D['d_tag'],'directed'=>$this->D['d_directed']);
    ...
    function loadgame($flag)
    {
    	if($this->P['make']!=true && chkCache($this->P['cp'],$this->P['cn'])){
    		...
    		$tp = MAC_ROOT_TEMPLATE."/" ;
    		if($flag=="detail"){ $tp .= $this->T['t_tpl_game']; }
    		elseif($flag=="play"){ $tp .= $GLOBALS['MAC']['game']['playisopen']==1 ? "gameplayopen.html" : $this->T["t_tpl_play"];  }
    		elseif($flag=="down"){ $tp .= $this->T['t_tpl_down']; }
    		elseif($flag=='rss') { $tp = MAC_ROOT.'/inc/map/rssid.html'; }
    		...
    		$this->P['sitetid'] = $this->D['d_type'];
    		$this->P['siteid'] = $this->D['d_id'];
    		$this->P['gametypeid'] = $this->T['t_id'];
    		$this->P['gametypepid'] = $this->T['t_pid'];
    
    		if(strpos($this->H,"[game:comment]")){
    			$commentH = loadFile(MAC_ROOT_TEMPLATE.'/home_comment.html');
    			$commentH = str_replace("{maccms:commentverify}", $GLOBALS['MAC']['other']['commentverify'] ,$commentH);
    			$this->H = str_replace('[game:comment]','<div id="comment" class="comment">'.$commentH.'</div>',$this->H);
    		}
    		$this->mark();
    		if(strpos($this->H,"{maccms:pages")){
    			$ps='MAC.Comment.Show(\'{url}\')';
    ...
    function replaceGame()
    {
    	$id = $this->D['d_id'];
    	$name = $this->D['d_name'];
    	$this->H = str_replace(array('[game:hits]','[game:fav]','[game:share]','[game:error]','[game:desktop]','[game:digg]','[game:scoremark1]','[game:scoremark2]'),array('<em id="hits">加载中</em><script>MAC.Hits("game","'.$id.'")</script>','<a target="_self" href="javascript:void(0)" onclick=\'MAC.Fav(document.URL,document.title);return false;\'>我要收藏</a>','<a target="_self" href="javascript:void(0)" onclick=\'MAC.Copy(document.title+"  " +document.URL);return false;\'>我要分享</a>','<a target="_self" href="javascript:void(0)" onclick=\'MAC.Error("game","'.$id.'","'.$name.'");return false;\'>我要报错</a>','<a target="_self" href="javascript:void(0)" onclick=\'MAC.Desktop("'.$name.'");return false;\'>保存到桌面</a>','<a target="_self" href="javascript:void(0)" class="digg_gameup">顶(<span>0</span>)</a><a target="_self" href="javascript:void(0)" class="digg_gamedown">踩(<span>0</span>)</a>','<script>MAC.Score.Show(0,"game",'.$id.');</script>','<script>MAC.Score.Show(1,"game",'.$id.');</script>'),$this->H);
    	 
    	if (strpos($this->H,"[game:history]")){
    		$this->H = str_replace('[game:history]', '<script>MAC.History.Insert("'.$name.'","'.$this->getLink('game','detail',$this->T,$this->D).'","'.$this->T['t_name'].'","'.$this->getLink('game','type',$typearr,$row,true).'","'.$this->D['d_pic'].'");</script>',$this->H);
    	}
    	if (strpos($this->H,"[game:textlink]")){
    		$this->H = str_replace("[game:textlink]", $this->getTypeText("game")."&nbsp;&nbsp;&raquo;&nbsp;&nbsp;<a href='". $this->getLink('game','detail',$this->T,$this->D) ."'>".$name."</a>" ,$this->H );
    	}
    	if (strpos($this->H,"[game:prelink]")){
    		$this->H = str_replace("[game:prelink]", $this->getPreNextLink('game',$id,0),$this->H);
    	}
    
    	if  (strpos($this->H,"[game:nextlink]")){
    		$this->H = str_replace("[game:nextlink]", $this->getPreNextLink('game',$id,1),$this->H);
    	}
    
    	$labelRule = buildregx("\[game:\s*([0-9a-zA-Z]+)([\s]*[len|style]*)[=]??([\da-zA-Z\-\\\/\:\s]*)\]","");
    	preg_match_all($labelRule ,$this->H,$matches2);
    	for($j=0;$j<count($matches2[0]);$j++)
    	{
    	$marktemp = $this->parse("game", $matches2[0][$j], $matches2[0][$j], $matches2[1][$j], $matches2[3][$j], $this->D, 0);
    	$this->H = str_replace($matches2[0][$j], $marktemp,$this->H);
    	}
    	unset($matches2);
    	$this->H = str_replace( "[game:".$flag."er]", "<script src=\"". MAC_PATH ."js/playerconfig.js\"></script><script src=\"". MAC_PATH ."js/player.js\"></script><script>MacPlayer.init('".$GLOBALS['MAC']['game']['suffix']."','".$flag."');</script>",$this->H);
    }
    ...
    function loadart()
    {
    	if($this->P['make']!=true && chkCache($this->P['cp'],$this->P['cn'])){
            ...
            $this->P['similar'] = array('tag'=>$this->D['a_tag']);
