...
if($method=='index')
{
	...
	$tpl->P['cp'] = 'app';
	$tpl->P['cn'] = 'game_index'.$tpl->P['pg'];
	//如果有缓存，会再次输出并终止
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);

	//加载“页面框架”模板(loadFile即直接取其内容)，并存入模板类的变量H中
	$tpl->H = loadFile(MAC_ROOT."/template/".$MAC['site']['templatedir']."/".$MAC['site']['htmldir']."/game_index.html");
	//连接数据库
...
elseif($method=='map')
{
	...
	$tpl->P['cn'] = 'game_map';
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);
	$tpl->H = loadFile(MAC_ROOT."/template/".$MAC['site']['templatedir']."/".$MAC['site']['htmldir']."/game_map.html");
	$db = new AppDb($MAC['db']['server'],$MAC['db']['user'],$MAC['db']['pass'],$MAC['db']['name']);
	$tpl->mark();
}
...
elseif($method=='list')
{
	...
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);
	$tpl->P['gametypeid'] = $tpl->P['id'];
	
	$tpl->T = $MAC_CACHE['gametype'][$tpl->P['gametypeid']];
...
{
	$tpl->P["siteaid"] = 13;
	$tpl->P['cp'] = 'gametopicindex';
	$tpl->P['cn'] = $tpl->P['pg'];
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);
	$db = new AppDb($MAC['db']['server'],$MAC['db']['user'],$MAC['db']['pass'],$MAC['db']['name']);
	$tpl->H = loadFile(MAC_ROOT."/template/".$MAC['site']['templatedir']."/".$MAC['site']['htmldir']."/game_topicindex.html");
	$tpl->mark();
	$tpl->pageshow();
}
...
elseif($method=='topic')
{
	...
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);
	$tpl->P['gametopicid'] = $tpl->P['id'];
	$tpl->T = $MAC_CACHE['gametopic'][$tpl->P['gametopicid']];
	if (!is_array($tpl->T)){ showMsg("获取数据失败，请勿非法传递参数","../"); }
	$db = new AppDb($MAC['db']['server'],$MAC['db']['user'],$MAC['db']['pass'],$MAC['db']['name']);
	$tpl->loadtopic('game');
	$tpl->pageshow();
...
elseif($method=='search')
{
    ...
    $tpl->P['typepid'] = 0;
	if(!isN($tpl->P["typeid"])){
		$typearr = $MAC_CACHE['gametype'][$tpl->P['typeid']];
		if (is_array($typearr)){
	...
	if(!isN($tpl->P["classid"])){
		$classarr = $MAC_CACHE['gameclass'][$tpl->P['classid']];
	...
	$tpl->H = loadFile(MAC_ROOT_TEMPLATE."/game_search.html");
	$tpl->mark();
	$tpl->pageshow();
    ...
    unset($colarr,$valarr);
    $linktype = $tpl->getLink('game','search','',array('typeid'=>$tpl->P['typepid']));
    $linkyear = $tpl->getLink('game','search','',array('year'=>''));
    $linkletter = $tpl->getLink('game','search','',array('letter'=>''));
    $linkarea = $tpl->getLink('game','search','',array('area'=>''));
    $linklang = $tpl->getLink('game','search','',array('lang'=>''));
    $linkclass = $tpl->getLink('game','search','',array('classid'=>''));
    
    
    $linkorderasc = $tpl->getLink('game','search','',array('order'=>'asc'));
    $linkorderdesc = $tpl->getLink('game','search','',array('order'=>'desc'));
    $linkbytime = $tpl->getLink('game','search','',array('by'=>'time'));
    $linkbyhits = $tpl->getLink('game','search','',array('by'=>'hits'));
    $linkbyscore = $tpl->getLink('game','search','',array('by'=>'score'));
...
elseif($method=='detail')
{
	...
	echoPageCache($tpl->P['cp'],$tpl->P['cn']);
	$db = new AppDb($MAC['db']['server'],$MAC['db']['user'],$MAC['db']['pass'],$MAC['db']['name']);
	$sql = "SELECT * FROM {pre}game WHERE d_hide=0 AND d_id=" . $tpl->P['id'];
	$row = $db->getRow($sql);
	if(!$row){ showMsg("获取数据失败，请勿非法传递参数",MAC_PATH); }
	if(!getUserPopedom($row["d_type"], "game")){ showMsg ("您没有权限浏览内容页", MAC_PATH."index.php?m=user-index.html"); }
	$tpl->T = $MAC_CACHE['gametype'][$row['d_type']];
	$tpl->D = $row;
	unset($row);
	$tpl->loadgame("detail");
	$tpl->replaceGame();
...
elseif($method=='play')
{
	...
	$tpl->replaceGame();
	$tpl->playdownlist('play');
	$tpl->H = str_replace('[game:playnum]',$tpl->P['num'],$tpl->H);
	$tpl->H = str_replace('[game:playsrc]',$tpl->P['src'],$tpl->H);
	$tpl->getUrlName('play');
	$tpl->H = str_replace('[game:playerinfo]', '<script>' .$tpl->getUrlInfo('play'). ' </script>'. "\n" ,$tpl->H);
	$tpl->H = str_replace('[game:player]', '<script src="'.$MAC['site']['installdir'].'js/playerconfig.js"></script><script src="'.$MAC['site']['installdir'].'js/player.js"></script>'. "\n" ,$tpl->H);
	$tpl->playdownlist ("down");
...
elseif($method=='down')
{
	...
	$tpl->replaceGame();
	$tpl->playdownlist ("down");
	$tpl->H = str_replace("[game:downnum]",$tpl->P["num"],$tpl->H);
	$tpl->H = str_replace("[game:downsrc]",$tpl->P["src"],$tpl->H);
	$tpl->getUrlName("down");
	$tpl->H = str_replace("[game:downinfo]", "<script>" .$tpl->getUrlInfo("down"). " </script>". "\n" ,$tpl->H);
	$tpl->H = str_replace('[game:downer]', '<script src="'.$MAC['site']['installdir'].'js/playerconfig.js"></script><script src="'.$MAC['site']['installdir'].'js/player.js"></script>'. "\n" ,$tpl->H);
	$tpl->playdownlist('play');
}
