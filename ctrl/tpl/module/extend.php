...
if($method=='pic')
{
	$path = $p['path']; $path = str_replace('\\','/',$path);
...
elseif($method=='picsyncvod')
{
	...
	if($nums>0){
		...
		while ($row = $db ->fetch_array($rs))
		{
		    	...
		    	$path = "upload/vod/" . getSavePicPath('vod') . "/";
...
elseif($method=='comment')
{
	...
	$arr=array(
		...
		array('a'=>'type','c'=>$type,'n'=>array('视频','文章'),'v'=>array(16,26))
	);
	foreach($arr as $a){
		$colarr=$a['n'];
		$valarr=$a['v'];
	...
	while ($row = $db ->fetch_array($rs))
	{
		...
		$plt->parse('rows_'.$rn,'list_'.$rn,true);
		if($row['c_type']==16) { $plt->set_if('rows_'.$rn,'isvod',true); } else { $plt->set_if('rows_'.$rn,'isvod',false); }
		if($row['c_hide']==1) { $plt->set_if('rows_'.$rn,'ishide',true); } else { $plt->set_if('rows_'.$rn,'ishide',false); }
	}
	
	$pageurl = '?m=extend-comment-pg-{pg}-hide-'.$hide.'-type-'.$type.'-wd-'.urlencode($wd);
	$pages = '共'.$nums.'条数据&nbsp;当前:'.$page.'/'.$pagecount.'页&nbsp;'.pageshow($page,$pagecount,8,$pageurl,'pagego(\''.$pageurl.'\','.$pagecount.')');
	$plt->set_var('pages', $pages );
}
