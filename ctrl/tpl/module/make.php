...
elseif($method=='info')
{
    ...
    elseif($page>$pagecount){
    	...
    	$rs = $db->query($sql);
    	$n=1;
    	echo '正在开始生成内容页面,共<font color=red>'.$datacount.'</font>条,分<font color=red>'.$pagecount.'</font>次生成,当前第<font color=red>'.$page.'</font>次<br>';
    	while($row = $db ->fetch_array($rs)){
    		if($rc){ $upids.=','; }
