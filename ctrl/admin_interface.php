...
function gettypere($flag,$tn)
{
    ...
    if ($flag=="art"){
    	...
    	$typearr = $GLOBALS['MAC_CACHE']['gametype'];
...
else if($ac=='game')
{
    ...
    if($MAC['collect']['game']['psernd']==1){
		$d_content = repPseRnd('game',$d_content,0);
    }
    if($MAC['collect']['game']['psesyn']==1){
		$d_content = repPseSyn('game',$d_content);
    }
    if($d_hits==0){
    	$d_hits = rand($MAC['collect']['game']['hitsstart'],$MAC['collect']['game']['hitsend']);
    }
    if($d_dayhits==0){
    	$d_dayhits = rand($MAC['collect']['game']['hitsstart'],$MAC['collect']['game']['hitsend']);
    }
    if($d_weekhits==0){
    	$d_weekhits = rand($MAC['collect']['game']['hitsstart'],$MAC['collect']['game']['hitsend']);
    }
    if($d_monthhits==0){
    	$d_monthhits = rand($MAC['collect']['game']['hitsstart'],$MAC['collect']['game']['hitsend']);
    }
    if($d_score==0 || $d_scoreall==0 || $d_scorenum==0){
		if($MAC['collect']['game']['score']==1){
			$d_scorenum = rand(1,500);
			$d_scoreall = $d_scorenum * rand(1,10);
			$d_score = round( $d_scoreall / $d_scorenum ,1);
		}
    ...
    $newtypeid = 0;
    $newtype = $d_type;
    if (!isNum($d_type)) { $newtypeid = gettypere("game", $d_type);} else{ $newtypeid = intval($d_type);}
    if ($newtypeid ==0) { echo $d_name . " " . $newtype . " 没有找到转换的分类err";exit; }
    $d_type = $newtypeid;
	...
	if(!isN($d_class)){
    	...
    	$d_class='';
    	$rc=false;
    	$c_pid = $MAC_CACHE['gametype'][$d_type]['t_pid'];
    	if($c_pid==0){ $c_pid=$d_type; }
    	
    	foreach($arr as $a){
    		$n = $db->getOne('select c_id from {pre}game_class where c_pid='.$c_pid.' and c_name=\''.$a.'\'');
    		if($rc){ $d_class.=','; }
    		if($n<1){
    			$db->Add('{pre}game_class',array('c_name','c_pid'),array($a,$c_pid));
    			$n=$db->insert_id();
    		}
    		$rc=true;
    		$d_class.=$n;
    ...
    $inrule = $MAC['collect']['game']['inrule'];
	$uprule = $MAC['collect']['game']['uprule'];
	$filter = $MAC['collect']['game']['filter'];
    if(strpos(','.$filter,$d_name)) { $des="数据在过滤单中,系统跳过采集err";exit; }
    if($d_tag=='' && $MAC['collect']['game']['tag']==1){
		$d_tag = getTag($d_name,$d_content);
	}
    
    $sql = "SELECT * FROM {pre}game WHERE d_name ='" .$d_name. "' ";
    if(strpos($inrule,'b')){ $sql.=' and d_type='.$d_type; }
	if(strpos($inrule,'c') ){ $sql.=' and d_year='.$d_year; }
	if(strpos($inrule,'d') ){ $sql.=' and d_area=\''.$d_area.'\''; }
	if(strpos($inrule,'e') ){ $sql.=' and d_lang=\''.$d_lang.'\''; }
    ...
    if (!$row){
        $resultdes = "新增数据ok";
        $db->Add ("{pre}game", array("d_name", "d_subname", "d_enname", "d_type","d_class", "d_state", "d_letter", "d_color", "d_pic","d_picthumb","d_picslide", "d_starring", "d_directed", "d_area", "d_year", "d_lang", "d_level", "d_stint","d_stintdown", "d_hits","d_dayhits","d_weekhits","d_monthhits", "d_content", "d_remarks","d_tag", "d_usergroup", "d_score","d_scoreall" , "d_scorenum","d_up","d_down","d_hide","d_duration","d_addtime", "d_time", "d_playurl", "d_playfrom", "d_playserver","d_playnote","d_downurl","d_downfrom", "d_downserver","d_downnote"), array($d_name, $d_subname, $d_enname, $d_type,$d_class, $d_state, $d_letter, $d_color, $d_pic,$d_picthumb,$d_picslide, $d_starring, $d_directed, $d_area, $d_year, $d_lang, $d_level, $d_stint,$d_stintdown, $d_hits,$d_dayhits,$d_weekhits,$d_monthhits,$d_content, $d_remarks,$d_tag, $d_usergroup,$d_score, $d_socreall, $d_scorenum,$d_up,$d_down,$d_hide,$d_duration, $d_addtime, $d_time, $d_playurl, $d_playfrom, $d_playserver,$d_playnote,$d_downurl, $d_downfrom, $d_downserver,$d_downnote));
    }
    else{
    	if($row['d_lock']==1){
        	$des = "数据已经锁定,系统跳过采集更新err";exit;
		...
		if(strpos(','.$uprule,'i')){ array_push($colarr,'d_lang'); array_push($valarr,$d_lang); }
		if(strpos(','.$uprule,'j')){
			if($MAC['collect']['game']['pic']==1){
				$st = strrpos($d_pic,'/');
				$fname = substring($d_pic,strlen($d_pic)-$st,$st+1);
				$path = "upload/game/" . getSavePicPath('') . "/";
				$thumbpath = "upload/gamethumb/" . getSavePicPath('gamethumb') . "/";
				$ps = savepic($d_pic,$path,$thumbpath,$fname,'game',$msg);
				if($ps){
					$d_pic=$path.$fname; $d_picthumb= $thumbpath.$fname; 
					array_push($colarr,'d_pic'); array_push($valarr,$d_pic);
					array_push($colarr,'d_picthumb'); array_push($valarr,$d_picthumb);
		...
		if(strpos(','.$uprule,'l')){ array_push($colarr,'d_tag'); array_push($valarr,$d_tag); }
		if(count($colarr)>0){
			$db->Update ("{pre}game",$colarr,$valarr,"d_id=".$row["d_id"]);
		}
    }
    unset($row);
    echo $resultdes;
