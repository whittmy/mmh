<?php
	...
	foreach($_FILES as $FILEa){
		...
		if(($id=='d_pic' || $id=='d_img1'||$id=='d_img2'||$id=='d_img3'||$id=='d_img4') && $MAC['upload']['thumb']==1){
		...
		if (($path=='vod'||$path=='game') && $action!="xht" && $MAC['upload']['ftp']==1){
			uploadftp($showdir,$fname);
			if($thumbst){ uploadftp($thumbdir,$fname); }
		}
		
		if($action=="xht"){
			echo '{"status":"true","err":"'.$errm.'","msg":"'.$MAC['app']['installdir'].$showdir.$fname.'"}';
		}
